...
def build_knowledge_base(raw_data_dir: str, output_dir: str):
    ...
    # Older chromadb raised ValueError when the collection was absent;
    # chromadb >=1.x raises chromadb.errors.NotFoundError instead.  Catch
    # both so a fresh (empty) DB doesn't crash the first build.
    try:
        client.delete_collection('school_info')
        print('  Cleared existing school_info collection.')
    except (ValueError, getattr(chromadb.errors, 'NotFoundError', Exception)):
