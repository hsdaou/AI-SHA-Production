...
class BrainNode(Node):
    def __init__(self):
        ...
        # Empty by default — the rule-based router needs no LLM model. Only set
        # this (to a CAPABLE model, >=1B) together with use_llm_router:=true.
        self.declare_parameter('router_model', '')
        self.declare_parameter('router_timeout', 30)
        # LLM router OFF by default. Empirical eval (2026-06-02): gemma3:270m
        # scored 0/18 usable in production (emits the wrong JSON key) and
        # ~chance even when parsed — it mode-collapses to one class and cannot
        # do 3-way intent classification at 270M. The rule-based heuristic
        # scores 18/18 on the same set, instantly, with no VRAM. Set true only
        # if you wire a CAPABLE router model (>=1B) into router_model.
        self.declare_parameter('use_llm_router', False)
        ...
        self.use_llm_router = self.get_parameter('use_llm_router').get_parameter_value().bool_value
    ...
    def _check_ollama(self):
        # With the LLM router off (default), brain_node does not use Ollama at
        # all — routing is rule-based. Skip the check entirely.
        if not self.use_llm_router:
            self.get_logger().info(
                'Router: rule-based heuristic (LLM router disabled). '
                'Set use_llm_router:=true with a capable model to enable.')
            return
        try:
    ...
    # Rule-based intent patterns (replace the broken gemma3:270m router).
    # ACTION = communication: messaging verbs, "send/shoot/... a note", or
    # "tell/let <someone> know".
    _ACTION_RE = re.compile(
        r'\b(whatsapp|email|e-mail|call|phone|text|message|msg|ping|notify|'
        r'remind|reminder)\b'
        r'|\b(send|shoot|drop|fire off|write|leave)\b.{0,20}\b(note|message|text|msg|email|word)\b'
        r'|\b(let|tell|inform|notify)\b.{0,25}\b(know|that)\b'
        r'|\b(tell|text|message|call|ping)\b\s+(my|the)?\s*'
        r'(mom|mum|dad|mother|father|coach|teacher|principal|friend|parents?)\b')
    # NAV = physical movement toward a place / come-here / follow.
    _NAV_RE = re.compile(
        r'\b(go|going|head|heading|walk|drive|roll|wheel|move|navigate|come|'
        r'proceed|get|take|bring|lead|guide)\b.{0,15}\b(to|over to|towards|here|there)\b'
        r'|\bfollow me\b|\bcome here\b|\btake me\b|\bbring me\b')

    def classify_intent(self, text):
        """Classify user intent — rule-based heuristic primary, LLM opt-in.

        The rule-based heuristic (_keyword_classify) is the primary router:
        on the eval set it scores 18/18 vs gemma3:270m's 0/18 usable, with
        zero latency and zero VRAM. The LLM path (_llm_classify) is consulted
        ONLY when use_llm_router is true AND the heuristic finds no rule match
        AND no ADMIN query is mid-flight (VRAM safety). With a 270M router the
        LLM path adds nothing, so it is disabled by default.
        """
        # ── Emergency pre-emption: bypass everything ──────────────────────
        ...
        # ── Primary: rule-based heuristic (instant, deterministic) ────────
        heuristic = self._keyword_classify(text)
        if heuristic is not None:
            return heuristic

        # ── Optional LLM fallback for genuinely ambiguous inputs ──────────
        if self.use_llm_router:
            # Proactively expire stale pending questions so a hung admin_node
            # doesn't latch the VRAM bypass indefinitely.
            ...
            # Skip the LLM if admin is mid-inference (Llama in VRAM -> OOM risk).
        ...
        # No rule matched (and LLM disabled or inconclusive): default to ADMIN,
        # the safe choice for a school-info assistant.
        self.get_logger().info('No rule matched -> ADMIN (default)')
        return {"intent": "ADMIN"}

    def _keyword_classify(self, text):
        text_lower = text.lower().strip()

        # Verb/pattern-based rules. ACTION before NAV (a "send a note" command
        # is communication even if it mentions a place). Scored 18/18 on the
        # eval set vs the old substring keywords' 12/18 (the gain is on
        # naturalistic phrasing: "head over to", "let mom know", "note to ...").
        if self._ACTION_RE.search(text_lower):
            self.get_logger().info('Heuristic -> ACTION')
            return {"intent": "ACTION"}
        if self._NAV_RE.search(text_lower):
            self.get_logger().info('Heuristic -> NAV')
            return {"intent": "NAV"}
        ...
        if text_lower.startswith(question_starters):
            self.get_logger().info('Question pattern -> ADMIN')
            return {"intent": "ADMIN"}
