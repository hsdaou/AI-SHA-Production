...
class TTSNode(Node):
    ...
    Subscribes to /robot_speech — the single canonical speech output topic.
    ...
    The ROS 2 subscription callback enqueues text instantly (non-blocking)
    and a single worker thread processes TTS sequentially. This avoids
    blocking the executor and prevents overlapping speech.

    Target architecture topics:
      Subscribes: /robot_speech     (std_msgs/String)
    ...
    def __init__(self):
        ...
        # Single canonical speech topic — all speech output routes through here.
        # /tts_text is deliberately NOT subscribed: brain_node tracks history via
        # /robot_speech, so anything published to /tts_text would bypass history.
        ...
        # Queue + single worker thread — keeps the ROS 2 executor unblocked
    ...
    def speak(self, msg):
        """ROS 2 callback — enqueues text and returns instantly."""
        text = self._sanitize_text(msg.data)
        if text:
            self._tts_queue.put(text)

    def _tts_worker_loop(self):
        """Single background thread that processes TTS requests sequentially."""
        while True:
    ...
    def _speak_impl(self, text):
        """Synthesize and play speech — runs in the worker thread only."""
        ...
        try:
            ...
            # Streaming pipeline: piper stdout → aplay stdin.
            # This is Piper's standard deployment pattern and provides the
            # lowest latency (audio starts before synthesis finishes).
            # If audio stutters under heavy CPU (Whisper + Piper on Pi 5),
            # consider piping to a temp .wav and playing the completed file
            # at the cost of ~1s extra latency.
            ...
            # Write text to piper's stdin manually and close it.
            # DO NOT use piper.communicate() here — it reads stdout internally,
            # racing with aplay for the same pipe and causing silent/clipped audio.
            # Closing piper.stdout in the parent lets aplay be the sole consumer.
            piper.stdin.write(text.encode('utf-8'))
            piper.stdin.flush()
            piper.stdin.close()    # EOF tells Piper to finish synthesis
            piper.stdout.close()   # Parent releases the pipe — aplay is sole reader
