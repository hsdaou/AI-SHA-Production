...
def generate_launch_description():
    ...
    def create_nodes(context):
        return [
            ...
            #
            # Memory budget on the 8GB Orin Nano (shared GPU/CPU RAM): the
            # YOLO TensorRT engine occupies the GPU, so llama3.2 (~2GB + KV
            # cache) cannot also fit in VRAM — cudaMalloc OOMs.
            # additional_env keeps admin_node off the GPU:
            #   CUDA_VISIBLE_DEVICES='' -> bge-small embeddings run on CPU
            #                              (this process's torch only; the
            #                               separate `ollama serve` keeps CUDA)
            #   OLLAMA_NUM_GPU         -> # of llama3.2 layers offloaded to GPU.
            #       0 = CPU-only. This is the RELIABLE production default.
            #       Measured on this 8GB board with the FULL cerebro_aisha
            #       stack up (YOLO TensorRT @ ~85% GPU):
            #         steady state: num_gpu<=12 OK (12 -> 12.3s vs 16.2s CPU,
            #                       ~24% faster); num_gpu>=16 -> cudaMalloc OOM.
            #         BUT during memory spikes (startup, KV-cache growth)
            #         EVEN num_gpu=4 OOMs -> llama-server segfaults.
            #       => No nonzero value is crash-safe under live vision load,
            #          and the steady-state win is only ~4s. Keep 0 for demos.
            #          (Vision OFF the ceiling is far higher: num_gpu=10 -> 8.2s.)
            #   OLLAMA_NUM_CTX=2048     -> bounded KV cache (model default is 131072)
            Node(
                package='aisha_brain',
                ...
                additional_env={
                    ...
                    'OLLAMA_NUM_GPU': context.perform_substitution(
                        LaunchConfiguration('llm_num_gpu')),
                    'OLLAMA_NUM_CTX': '2048',
                },
                parameters=[{
    ...
    return LaunchDescription([
        # llama3.2:1b (CPU) answers in ~8-15s vs ~60s for the 3B on this
        # memory-constrained Jetson. Override with llm_model:=llama3.2 for
        # higher answer quality at the cost of latency.
        DeclareLaunchArgument('llm_model',    default_value='llama3.2:1b'),
        # GPU layers for the admin RAG model. Default 0 (CPU-only) is the
        # safe, demo-tested setting. Raise to experiment with partial offload
        # (e.g. llm_num_gpu:=10) — but only while load-testing with YOLO up,
        # since full vision + GPU LLM OOMs on this 8GB board.
        DeclareLaunchArgument('llm_num_gpu',  default_value='0'),
