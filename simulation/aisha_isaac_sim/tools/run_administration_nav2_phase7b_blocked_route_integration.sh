...
RUNTIME_DIR=$(mktemp -d -t aisha-nav2-phase7b-XXXXXX)
BRIDGE_PID=""
NAV2_PID=""
...
"$SCRIPT_DIR/run_administration_nav2_phase7b_blocked_route_bridge.sh" \
...
if ! kill -0 "$BRIDGE_PID" 2>/dev/null; then
  ...
  echo "AI-SHA Phase 7B blocked-route bridge exited before Nav2 startup" >&2
  exit 1
fi

"$SCRIPT_DIR/run_administration_nav2_phase6_servers.sh" \
...
if ! kill -0 "$NAV2_PID" 2>/dev/null; then
  ...
  echo "AI-SHA Phase 7B Nav2 servers exited before mission startup" >&2
  exit 1
fi

if ! "$SCRIPT_DIR/run_administration_nav2_phase7b_blocked_route_mission.sh"; then
...
python3 "$SCRIPT_DIR/validate_administration_nav2_phase7b_blocked_route_integration.py"
