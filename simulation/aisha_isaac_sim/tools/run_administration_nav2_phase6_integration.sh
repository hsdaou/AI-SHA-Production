...
RUNTIME_DIR=$(mktemp -d -t aisha-nav2-phase6-XXXXXX)
BRIDGE_PID=""
NAV2_PID=""
...
"$SCRIPT_DIR/run_administration_nav2_phase6_bridge.sh" \
...
if ! kill -0 "$BRIDGE_PID" 2>/dev/null; then
  ...
  echo "AI-SHA Phase 6 measured bridge exited before Nav2 startup" >&2
  exit 1
fi
...
if ! kill -0 "$NAV2_PID" 2>/dev/null; then
  ...
  echo "AI-SHA Phase 6 Nav2 servers exited before mission startup" >&2
  exit 1
fi

if ! "$SCRIPT_DIR/run_administration_nav2_phase6_mission.sh"; then
...
python3 "$SCRIPT_DIR/validate_administration_nav2_phase6_integration.py"
