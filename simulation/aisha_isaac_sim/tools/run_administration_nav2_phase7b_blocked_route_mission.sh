...
exec python3 "$SCRIPT_DIR/run_administration_nav2_mission.py" \
  ...
  --control-stack nav2_mapped_doorway_phase7b_blocked_route_replanning_safety \
  --waypoint-timeout-s 180 \
  --output "$SIM_ROOT/results/administration_nav2_phase7b_blocked_route_mission.json" \
  "$@"
